...
+function ($) {
    ...
    Dialog.prototype.close = function(dialog) {
    	if(typeof(dialog) != "object" || typeof(dialog) == "undefined" || typeof(dialog) == "null" || dialog == 'null' || dialog == null)
    	{
    		return false;
    	}
        var that        = this,
...
+function ($) {
    ...
    Bjuiajax.prototype.doajaxchecked = function(options) {
        ...
        var todo = function() {
            ...
            if (!$target || !$target.length) {
                if (that.tools.getTarget() == Bjuiajax.NAVTAB || options.isNavtab) {
                    $target = $.CurrentNavtab
                } else {
                    $target = $.CurrentDialog
                }
      ...
      //parse default tags
        ...
        if (tags) {
            ...
            $.each(tags, function(index, value) {
                that.tools.createTag(value, value)
            });
            $element.val('')
        }
            ...
            filter: function(files) {
                ...
                if (typeArray.length > 0) {
                    for (var i = 0; i < files.length; i++) {
                        ...
                        if ($.inArray(thisFile.name.split('.').pop().toLowerCase(), typeArray) >= 0 || options.fileTypeExts == '*' ) {
                            arr.push(thisFile)
                        } else{
...
+function ($) {
    ...
    $(document).on(BJUI.eventType.initUI, function(e) {
            ...
            $box.find('[data-toggle="ztree"]').each(function() {
                ...
                function _onNodeCreated(event, treeId, treeNode) {
                    if (treeNode.faicon) {
                        ...
                        //如果菜单是等级是1并且设定了快速目录
                        if( $('#'+ treeNode.tId).attr('class')=='level1' && treeNode.quickmenu){
                            $('#'+ treeNode.tId).prepend('<a style="width:20%;float:right;line-height:24px;text-align:right;" title="'+treeNode.quickmenutitle+'快捷菜单" class="'+treeNode.quickmenuclass+'" onClick="QuickMenu(this,'+treeNode.id+')" href="javascript:;"></a>');
                        }
        ...
        /* ========================================================================
         * @description ueditor
         * @author 未梦 <1747699213@qq.com>
         * @Blog http://www.weimengcms.com
         * 使用方法：<textarea style="样式" name="" data-name="" data-toggle="ueditor">content</textarea>
         * ======================================================================== */
        if(UE){
	       $box.find('[data-toggle="ueditor"]').each(function(){
	    	   var textName = $(this).data('name');
	    	   ...
	    	   if( textId == '' || typeof(textId) == "undefined" ){
	    		   textId = parseInt(Math.random() * 1000);
	    		}
	    	   textId = 'ueditor'+textId;

	           $(this).after('<script style="'+textCss+'" id="'+textId+'" name="'+textName+'" type="text/plain">'+textVal+'</script>');
	           $(this).remove();
	           ue = UE.getEditor(textId);
	        })
        }
        ...
        * @Blog http://www.weimengcms.com
        * ======================================================================== */
       $box.find('[data-toggle="autocomplete"]').each(function(){
    	   var textWidth = $(this).width();
    	   var textUrl = $(this).data('url');
    	   var textCallBack = $(this).data('callback');
    	   //禁用浏览器自动提示
    	   $(this).attr("autocomplete","off");
    	   $(this).bigAutocomplete({
    			width:textWidth+10,
    			url:textUrl,
    			/*data:[{title:"中国移动网上营业厅"},],*/
    			callback:function(data){
    				if( textCallBack != '' && typeof(textCallBack) != "undefined"){
    					window[textCallBack](data);
    				}
    				//alert(data.key);	
    			}
    		});
 ...
 * 生成翻页列表。当前页数,总页数
 */
function createPage(page,sum){
	var preCss = nextCss = html = lastCss = lastNextCss = indexDis = lastDis = '';
	var indexPage = prePage = lastPage = lastNextPage = 0;
	page = parseInt(page);
	sum = parseInt(sum);
	if( page == 1){
		indexDis = 'disabled';
	...
	if( page == sum){
		...
		lastNextCss = 'display: none;';
	}
	html = '<li class="j-first '+indexDis+'" data-page="'+indexPage+'"><a class="first" href="javascript:;" style="'+preCss+'"><i class="fa fa-step-backward"></i> 首页</a>'+
		'<span class="first" style="'+nextCss+'"><i class="fa fa-step-backward"></i> 首页</span></li>'+
		'<li class="j-prev '+indexDis+'" data-page="'+prePage+'"><a class="previous" href="javascript:;" style="'+preCss+'"><i class="fa fa-backward"></i> 上一页</a>'+
		'<span class="previous" style="'+nextCss+'"><i class="fa fa-backward"></i> 上一页</span></li>';
	...
	for(var i=startPage;i<=forSum;i++){
		selected = '';
		if(i == page){
			selected = 'selected';
		}
		html += '<li class="j-num '+selected+'" data-page="'+i+'"><a href="javascript:;">'+i+'</a></li>';
	}
	html += '<li class="j-next '+lastDis+'" data-page="'+lastNextPage+'"><a class="next" href="javascript:;" style="'+lastCss+'">下一页 <i class="fa fa-forward"></i></a>'+
	'<span class="next" style="'+lastNextCss+'">下一页 <i class="fa fa-forward"></i></span></li>'+
	'<li class="j-last '+lastDis+'" data-page="'+lastPage+'"><a class="last" href="javascript:;" style="'+lastCss+'">末页 <i class="fa fa-step-forward"></i></a>'+
	'<span class="last" style="'+lastNextCss+'">末页 <i class="fa fa-step-forward"></i></span></li>';
	return html;
}