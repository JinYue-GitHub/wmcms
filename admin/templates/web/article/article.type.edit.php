<div class="bjui-pageContent">
    <form action="index.php?a=yes&c=article.type&t=<?php echo $type;?>" data-reload="false" data-toggle="validate" method="post" <?php if($d) { echo 'data-callback="'.$cFun.'"';}?>>
	<input name="type_id" id="<?php echo $cFun.$type;?>type_id" type="hidden" class="input-text" value="<?php echo $id;?>">
		<fieldset>
			<legend>文章分类编辑</legend>
            ...
            <div class="tab-content">
            	<div class="tab-pane fade active in" id="<?php echo $cFun.$type;?>base">
					<table class="table table-border table-bordered table-bg table-sort">
		                ...
		                <tr>
					      <td valign="top"><b>分类状态：</b></td>
		                  <td valign="top">
		                   <select data-toggle="selectpicker" name="type[type_status]">
		                        <?php 
			                    foreach ($statusArr as $k=>$v)
			                    {
			                       $selected = str::CheckElse( $k, C('type_status',null,'data') , 'selected=""');
					    ...
					    <tr>
					      <td valign="top"><b>允许投稿：</b></td>
					      <td valign="top">
					      		<?php
								$input['type'] = 'radio';
								$input['name'] = 'type[type_add]';
								$input['value'] = C('type_add',null,'data');
								$input['option'] = array('0'=>'禁止投稿','1'=>'允许投稿');
					...
					<table class="table table-border table-bordered table-bg table-sort">
					    <tr>
					      ...
					      <td valign="top">
					      	...
					      	<input id="temp_tiname" name="temp[temp_tiname]" type="text" value="<?php echo C('tiname',null,'temp');?>" data-toggle="lookup" data-url="index.php?c=system.templates.lookup&module=article&page=tindex&name=tiname&tid=titempid" data-title="选择当前文章分类首页模版" size="15" data-width="700" data-height="500">
					      	<a class="btn btn-default" href="index.php?c=system.templates.edit&t=add&module=article&page=tindex&name=tiname&tid=titempid" data-toggle="dialog" data-title="上传新的文章分类首页模版" data-width="540" data-height="450" ><i class="fa fa-cloud-upload">&nbsp;</i>上传模板</a>
					      </td>
						</tr>
					    <tr>
					      <td valign="top" width="150"><b>文章列表页模版：</b></td>
					      <td valign="top">
					      	<input id="type_tempid" name="type[type_tempid]" type="hidden" value="<?php echo C('type_tempid',null,'data');?>">
					      	<input id="temp_name" name="temp[temp_name]" type="text" value="<?php echo C('name',null,'temp');?>" data-toggle="lookup" data-url="index.php?c=system.templates.lookup&module=article&page=list&name=name&tid=tempid" data-title="选择当前文章分类列表页模版" size="15" data-width="700" data-height="500">
					      	<a class="btn btn-default" href="index.php?c=system.templates.edit&t=add&module=article&page=list&name=name&tid=tempid" data-toggle="dialog" data-title="上传新的文章列表页模版" data-width="540" data-height="450" ><i class="fa fa-cloud-upload">&nbsp;</i>上传模板</a>
					      </td>
						</tr>
					    <tr>
					      <td valign="top"><b>文章内容页模版：</b></td>
					      <td valign="top">
					      	<input id="type_ctempid" name="type[type_ctempid]" type="hidden" value="<?php echo C('type_ctempid',null,'data');?>">
					      	<input id="temp_cname" name="temp[temp_cname]" type="text" value="<?php echo C('cname',null,'temp');?>" data-toggle="lookup" data-url="index.php?c=system.templates.lookup&module=article&page=content&name=cname&tid=ctempid" data-title="选择当前文章分类内容页模版" size="15" data-width="700" data-height="500">
					      	<a class="btn btn-default" href="index.php?c=system.templates.edit&t=add&module=article&page=content&name=cname&tid=ctempid" data-toggle="dialog" data-title="上传新的文章内容页模版" data-width="540" data-height="450" ><i class="fa fa-cloud-upload">&nbsp;</i>上传模板</a>
					      </td>
						</tr>
					    <tr>
...
function <?php echo $cFun;?>(json){
	var op = articleTypeListGetOp();
	var tabid = 'article-type-list';
	op['id'] = tabid;
