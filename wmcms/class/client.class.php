<?php
/**
* 获取客户端操作系统，浏览器，语言，IP，IP归属地等 
* 
* @version        $Id: client.php 2015年8月9日 21:43  weimeng
* @package        WMCMS
...
class client
{
    ...
    function CID_unix_detect_os($ua) {
        ...
        if (preg_match('/Linux/i', $ua)) {
            ...
            if (preg_match('#Debian#i', $ua)) {
                ...
                $matches = explode(';',$ua);
                $os_code = "kindle";
                $matches2 = explode(')',GetKey($matches,'4'));
                $os_name = GetKey($matches,'2').$matches2[0];
            ...
            $os_name = "Mac OS";
            $os_code = "macos";
            if(count(explode(7,GetKey($matches,'1')))>1) $matches[1] = 'Lion '.GetKey($matches,'1');
            elseif(count(explode(8,GetKey($matches,'1')))>1) $matches[1] = 'Mountain Lion '.GetKey($matches,'1');
            $os_ver = "X ".GetKey($matches,'1');
    ...
    function CID_pda_detect_os($ua) {
        ...
        if (preg_match('#PalmOS#i', $ua)) {
            ...
            $pda_name = "Sharp Zaurus " . GetKey($matches,'1');
            $pda_code = "zaurus";
            $pda_ver = GetKey($matches,'1');
    ...
    function CID_detect_browser($ua) {
        ...
        if (preg_match('#MovableType[ /]([a-zA-Z0-9.]+)#i', $ua, $matches)) {
            ...
            $browser_name = 'Drupal';
            $browser_code = 'drupal';
            $browser_ver = count($matches) > 0 ? GetKey($matches,'1') : "";
            ...
            $browser_ver = '4'.GetKey($matches,'2');
            if (preg_match('/Windows/i', $ua)) {
                list($os_name, $os_code, $os_ver) = $this->CID_windows_detect_os($ua);
            } else {
                list($os_name, $os_code, $os_ver) = $this->CID_unix_detect_os($ua);
            ...
            $browser_code = 'sogou';
            $browser_ver = '2'.GetKey($matches,'1');
            if (preg_match('/Windows/i', $ua)) {
                list($os_name, $os_code, $os_ver) = $this->CID_windows_detect_os($ua);
            } else {
                list($os_name, $os_code, $os_ver) = $this->CID_unix_detect_os($ua);
			...
			if (preg_match('#OPR/([a-zA-Z0-9.]+)#i', $ua, $matches)) {
				$browser_name = 'Opera Chrome'; $browser_code = 'opera'; $browser_ver = GetKey($matches,'1');
			}else{
				$browser_name = 'Google Chrome'; $browser_code = 'chrome'; $browser_ver = GetKey($matches,'1');
			}
            if (preg_match('/Windows/i', $ua)) {
                list($os_name, $os_code, $os_ver) = $this->CID_windows_detect_os($ua);
            } else {
  ...
  function get_onlineip() {
   ...
   function Get_Ip_Addr()
   {
        ...
        for($i=0;$i<$count;$i++){   
            if(!preg_match("/^(10|172\.16|192\.168)\./i",GetKey($ips,$i))){//排除局域网ip
                $ip = GetKey($ips,$i);
                break;    
            }  
        }  
