...
class SaeTClientV2
{
	 ...
	 * @param 参数keyword与capital二者必选其一，且只能选其一。按首字母capital查询时，必须提供province参数。
	 * @access public
	 * @return array
	 */
	 ...
	 * @param 参数province与city二者必选其一<br />
	 * @param 参数id为空，则为新建职业信息，参数company变为必填项，参数id非空，则为更新，参数company可选
	 * @return array
	 */
...
class OtherLogin{
	...
	function __construct($data=array())
	{
		...
		$this->weibo = new SaeTOAuthV2($this->apikey,$this->secret);
	}
	...
	{
		//获得登录地址并且跳转
		$loginurl = $this->weibo->getAuthorizeURL($this->backurl);
		return $loginurl;
	}
	...
	function GetUserInfo($token='',$openid='')
	{
		...
		if ( isset($_REQUEST['code']) )
		{
			...
			{
				//获取用户信息
				$rs = $this->weibo->getAccessToken('code',$keys);
				
				$data['type'] = '新浪微博';
				$data['openid'] = $rs['uid'];
				$data['nickname'] = $rs['uid'];
				$data['unionid'] = isset($rs['unionid'])?$rs['unionid']:'';
				return $data;
			}
